...
void StrategyMRUEnqueue(volatile BufferDesc *buf) 
{
	...
	//Assert(buf->mruNext == MRU_NOT_IN_LIST);
	...
	buf->mruPrev = StrategyControl->mruTail;
	
	StrategyControl->mruTail = buf->buf_id;
	
	if (buf->mruPrev != MRUPREV_HEAD_OF_LIST)
	{
		Assert((BufferDescriptors + buf->mruPrev)->mruNext == MRUNEXT_END_OF_LIST);
		(BufferDescriptors + buf->mruPrev)->mruNext = buf->buf_id;
	} 
	else
	{
		StrategyControl->mruHead = buf->buf_id;
